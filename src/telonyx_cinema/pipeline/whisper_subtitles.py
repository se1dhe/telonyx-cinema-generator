...
from telonyx_cinema.config.model_config import COMPUTE_TYPE, ENABLE_WHISPER, MODEL_DEVICE, WHISPER_MODEL
from telonyx_cinema.pipeline.subtitle_builder import write_placeholder_ass


def extract_wav(video_path: str, wav_path: str) -> None:
    process = subprocess.run(['ffmpeg', '-y', '-i', video_path, '-vn', '-ac', '1', '-ar', '16000', wav_path], stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
...
def build_subtitles(video_path: str, output_ass: str, fallback_title: str = 'TELONYX CINEMA') -> str:
    ...
    try:
        ...
        wav_path = str(Path(output_ass).parent / 'speech.wav')
