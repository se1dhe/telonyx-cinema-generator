...
def render_segments(
    ...
    rendered_files = render_segment_files(video_path, segments, work_dir, enable_color, color_preset, enable_centering, enable_effects, effect_intensity, transitions_enabled, transition_style)
...
def render_segment_files(
    ...
    for index, segment in enumerate(segments):
        ...
        render_one_segment(video_path, segment, str(output), enable_color, color_preset, enable_centering, enable_effects, effect_intensity, transitions_enabled, transition_style)
...
def render_one_segment(
    ...
    vf = build_smart_filter(video_path, segment, enable_color, color_preset, enable_centering, enable_effects, effect_intensity, transitions_enabled, transition_style)
    ...
    cmd = [
        'ffmpeg', '-y',
        '-ss', str(segment['start']), '-t', str(source_duration), '-i', video_path,
        '-vf', vf_with_speed,
        '-an',
        '-c:v', 'libx264',
        '-preset', 'veryfast',
