...
APP_VERSION = "dialogue-shorts-v12-publishing-mvp-2026-05-18"
...
def write_title_ass(path: Path, movie_title: str, movie_year: str, duration: float) -> None:
    # Важно: весь текстовый титр теперь один ASS-dialogue block.
    # TXC UKRAINE + название + год движутся как единый объект.
    # Правый вертикальный watermark убран по требованию: остается только основной титр слева.
    ...
    cyan = "&H00EED322"      # RGB 34,211,238 в ASS BGR
    cyan_soft = "&H66EED322"
...
def render_video(job_id: str) -> None:
    ...
    try:
        ...
        write_state(job_id, {"progress": 22, "duration": duration, "message": "Готовлю TXC Ukraine титр без watermark"})
