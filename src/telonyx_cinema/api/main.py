...
function tiktokPollJob(id){fetch('/api/tiktok-jobs/'+id,{cache:'no-store'}).then(r=>r.json()).then(job=>{tiktokSetProgress(job.progress||0);tiktokStatus.textContent=job.message||job.status;tiktokLog.textContent=JSON.stringify(job,null,2);if(job.status==='done'){currentTikTokJobId=id;tiktokSetToast('Готово! Видео с логотипом l2watcher','ok');tiktokDownload.href=job.download_url;tiktokDownload.style.display='block';if(job.suggested_tags&&job.suggested_tags.length){tiktokTags.style.display='block';tiktokTags.innerHTML='<b>Теги для TikTok:</b><br>'+job.suggested_tags.map(t=>'<span>#'+esc(t)+'</span>').join('')}tiktokRenderBtn.disabled=false;return}if(job.status==='failed'){tiktokSetToast('Ошибка: '+(job.message||'unknown'),'err');tiktokRenderBtn.disabled=false;return}setTimeout(()=>tiktokPollJob(id),1500)}).catch(e=>{tiktokSetToast('Ошибка статуса: '+e.message,'err');tiktokRenderBtn.disabled=false})}
