...
def render_tiktok_video(job_id: str) -> None:
    ...
    try:
        ...
        filter_complex = (
            ...
            f"rotate={angle}:ow=1.5*iw:oh=1.5*oh:c=black@0[logo];"
...
const tiktokForm=document.getElementById('tiktokForm'),tiktokVideoInput=document.getElementById('tiktokVideoInput'),tiktokRenderBtn=document.getElementById('tiktokRenderBtn'),tiktokToast=document.getElementById('tiktokToast'),tiktokProgress=document.getElementById('tiktokProgress'),tiktokStatus=document.getElementById('tiktokStatus'),tiktokLog=document.getElementById('tiktokLog'),tiktokDownload=document.getElementById('tiktokDownload');
...
function startTikTokRender(){if(!tiktokVideoInput.files.length)return tiktokSetToast('Выбери видеофайл.','err');tiktokDownload.style.display='none';tiktokRenderBtn.disabled=true;tiktokSetToast('Загружаю файл...','warn');tiktokSetProgress(3);const fd=new FormData(tiktokForm);fetch('/api/tiktok-jobs',{method:'POST',body:fd}).then(r=>r.json()).then(r=>{tiktokSetToast('Задача создана: '+r.job_id,'ok');tiktokPollJob(r.job_id)}).catch(e=>{tiktokSetToast('Ошибка старта: '+e.message,'err');tiktokRenderBtn.disabled=false})}
