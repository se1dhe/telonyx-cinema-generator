...
APP_VERSION = "dialogue-shorts-v2-premium-persistent-title-2026-05-18"
...
def ass_filter_path(path: Path) -> str:
    # Для ffmpeg subtitles-фильтра нужно экранировать обратные слэши, двоеточия и апострофы.
    return str(path).replace("\\", "\\\\").replace(":", "\\:").replace("'", "\\'")


def write_title_ass(path: Path, movie_title: str, movie_year: str, duration: float) -> None:
    # Премиальный титр теперь живёт весь ролик:
    # 1) выкатывается в начале;
    # 2) висит на протяжении всего шортса;
    # 3) закатывается только под самый конец.
    ...
    accent_hidden_x = -90
    accent_y = 1455
    accent_shape = r"{\p1}m 0 0 l 8 0 l 8 112 l 0 112{\p0}"
    title_text = f"{{\\fsp2.6\\blur0.45}}{title}\\N{{\\fs34\\fsp4\\alpha&H18&}}{year}"
...
Style: Title,DejaVu Sans,46,&H00F7F7F4,&H000000FF,&HAA050505,&H66000000,-1,0,0,0,100,100,0,0,1,2.2,1.4,1,70,70,300,1
Style: Accent,DejaVu Sans,42,&H0022D3EE,&H000000FF,&H0022D3EE,&H00000000,-1,0,0,0,100,100,0,0,1,0,0,7,0,0,0,1

[Events]
Format: Layer, Start, End, Style, Name, MarginL, MarginR, MarginV, Effect, Text
Dialogue: 1,{ass_time(0.00)},{ass_time(outro_start)},Accent,,0,0,0,,{{\\move({accent_hidden_x},{accent_y},{accent_x},{accent_y},0,{enter_end_ms})\\fad(130,0)}}{accent_shape}
Dialogue: 2,{ass_time(0.00)},{ass_time(outro_start)},Title,,0,0,0,,{{\\move({hidden_x},{title_y},{title_x},{title_y},0,{enter_end_ms})\\fad(140,0)}}{title_text}
Dialogue: 1,{ass_time(outro_start)},{ass_time(duration)},Accent,,0,0,0,,{{\\move({accent_x},{accent_y},{accent_hidden_x},{accent_y},{outro_move_start_ms},{outro_move_end_ms})\\fad(0,180)}}{accent_shape}
Dialogue: 2,{ass_time(outro_start)},{ass_time(duration)},Title,,0,0,0,,{{\\move({title_x},{title_y},{hidden_x},{title_y},{outro_move_start_ms},{outro_move_end_ms})\\fad(0,200)}}{title_text}
"""
...
def render_video(job_id: str) -> None:
    ...
    try:
        ...
        write_state(job_id, {"progress": 22, "duration": duration, "message": "Готовлю премиальный титр на весь шортс"})
