...
APP_VERSION = "dialogue-shorts-v6-minimal-cinema-title-2026-05-18"
...
def write_title_ass(path: Path, movie_title: str, movie_year: str, duration: float) -> None:
    # Минималистичный кино-титр: только название и год.
    # Без линий, иконок, HUD, REC и декоративного шума.
    # Плотная типографика: год стоит близко к названию, как у профессиональных opening/lower-title credits.
    title = ass_escape((movie_title.strip() or "MOVIE").upper())
    year = ass_escape(movie_year.strip() or "YEAR")

    enter_end_ms = 760
    outro_start = max(0.8, duration - 1.18)
...
Style: MovieTitle,DejaVu Sans,42,&H00F4F0E7,&H000000FF,&HB2000000,&H00000000,-1,0,0,0,100,100,0,0,1,1.2,0.45,1,0,0,0,1
Style: MovieYear,DejaVu Sans,22,&HA8FFFFFF,&H000000FF,&H8A000000,&H00000000,-1,0,0,0,100,100,0,0,1,0.45,0.18,1,0,0,0,1
Style: Mark,DejaVu Sans,22,&H72FFFFFF,&H000000FF,&H66000000,&H00000000,-1,0,0,0,100,100,0,0,1,0.8,0.4,5,0,0,0,1

[Events]
Format: Layer, Start, End, Style, Name, MarginL, MarginR, MarginV, Effect, Text
Dialogue: 2,{ass_time(0.00)},{ass_time(outro_start)},MovieTitle,,0,0,0,,{{\\move({hidden_x},{y_title},{x},{y_title},0,{enter_end_ms})\\fsp1.1\\blur0.18\\fad(120,0)}}{title}
Dialogue: 2,{ass_time(0.05)},{ass_time(outro_start)},MovieYear,,0,0,0,,{{\\move({hidden_x},{y_year},{x + 1},{y_year},0,{enter_end_ms})\\fsp5.2\\blur0.1\\fad(120,0)}}{year}

Dialogue: 2,{ass_time(outro_start)},{ass_time(duration)},MovieTitle,,0,0,0,,{{\\move({x},{y_title},{hidden_x},{y_title},0,{outro_move_end_ms})\\fsp1.1\\blur0.18\\fad(0,190)}}{title}
Dialogue: 2,{ass_time(outro_start)},{ass_time(duration)},MovieYear,,0,0,0,,{{\\move({x + 1},{y_year},{hidden_x},{y_year},0,{outro_move_end_ms})\\fsp5.2\\blur0.1\\fad(0,190)}}{year}
...
def render_video(job_id: str) -> None:
    ...
    try:
        ...
        write_state(job_id, {"progress": 22, "duration": duration, "message": "Готовлю минималистичный кино-титр"})
