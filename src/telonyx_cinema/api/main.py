...
APP_VERSION = "dialogue-shorts-v13-no-form-refresh-2026-05-18"
...
(function(){
  ...
  const packageImages = document.getElementById('packageImages');
  const telegramPost = document.getElementById('telegramPost');
  const tiktokPost = document.getElementById('tiktokPost');
  const youtubePost = document.getElementById('youtubePost');
  const publishTelegram = document.getElementById('publishTelegram');
  const publishResult = document.getElementById('publishResult');

  function setToast(text, cls){ toast.className = 'toast ' + (cls || ''); toast.textContent = text; }
  function setProgress(value){ progress.style.width = (value || 0) + '%'; }
  function esc(value){ return String(value || '').replace(/[&<>]/g, function(ch){ return {'&':'&amp;','<':'&lt;','>':'&gt;'}[ch]; }); }

  function refreshPreview(){
    previewTitle.innerHTML = '<b>TXC UKRAINE</b><strong>' + esc(movieTitle.value || 'MOVIE') + '</strong><small>' + esc(movieYear.value || 'YEAR') + '</small>';
  }
  ...
  function pollJob(jobId){
    fetch('/api/jobs/' + jobId, {cache:'no-store'})
      ...
      .then(function(job){
        ...
        if (job.status === 'done') {
          currentJobId = jobId;
          setToast('Готово. Можно скачать итоговый MP4 или генерировать пост.', 'ok');
        ...
        if (job.status === 'failed') {
          setToast('Ошибка рендера: ' + (job.message || 'unknown'), 'err');
          renderBtn.disabled = false;
          return;
        }
      ...
      .catch(function(error){
        setToast('Ошибка статуса: ' + error.message, 'err');
        renderBtn.disabled = false;
      });
  }

  function renderPackage(pack){
    packageStatus.className = 'toast ' + (pack.status === 'ready' ? 'ok' : pack.status === 'failed' ? 'err' : 'warn');
    packageStatus.textContent = pack.message || pack.status;
    packageImages.innerHTML = (pack.images || []).map(function(img){ return '<img src="' + img.url + '" alt="' + esc(img.alt_text_uk) + '">'; }).join('');
    telegramPost.innerHTML = esc(pack.telegram_text_uk || '');
    tiktokPost.innerHTML = esc((pack.tiktok_title || '') + '\n\n' + (pack.tiktok_description || '') + '\n\n' + (pack.tiktok_hashtags || []).join(' '));
    youtubePost.innerHTML = esc((pack.youtube_title || '') + '\n\n' + (pack.youtube_description || '') + '\n\n' + (pack.youtube_hashtags || []).join(' '));
    publishResult.textContent = JSON.stringify(pack.publish_results || pack.generator_meta || {}, null, 2);
  }

  function pollPackage(packageId){
    fetch('/api/publish-packages/' + packageId, {cache:'no-store'})
      .then(parseJsonResponse)
      .then(function(pack){
        renderPackage(pack);
        if (pack.status === 'queued' || pack.status === 'generating') {
          window.setTimeout(function(){ pollPackage(packageId); }, 1500);
        }
      })
      .catch(function(error){
        packageStatus.className = 'toast err';
        packageStatus.textContent = 'Ошибка пакета: ' + error.message;
      });
  }
  ...
  function startRender(){
    ...
    if (!videoInput.files || !videoInput.files.length) { setToast('Выбери видеофайл.', 'err'); return; }
    if (!movieTitle.value.trim()) { setToast('Введи название фильма.', 'err'); return; }
    if (!movieYear.value.trim()) { setToast('Введи год выхода.', 'err'); return; }

    download.style.display = 'none';
    generateBtn.style.display = 'none';
    publishBox.style.display = 'none';
    renderBtn.disabled = true;
    setToast('Загружаю файл и запускаю рендер...', 'warn');
    setProgress(3);
    ...
    fetch('/api/jobs', { method:'POST', body: formData })
      ...
      .then(function(result){
        setToast('Задача создана: ' + result.job_id, 'ok');
        pollJob(result.job_id);
      })
      .catch(function(error){
        setToast('Ошибка старта: ' + error.message, 'err');
        renderBtn.disabled = false;
      });
  }
  ...
  function startGenerate(){
    ...
    packageStatus.className = 'toast warn';
    packageStatus.textContent = 'Запускаю генерацию контент-пакета...';

    fetch('/api/jobs/' + currentJobId + '/generate-package', { method:'POST' })
      .then(parseJsonResponse)
      .then(function(result){
        currentPackageId = result.package_id;
        pollPackage(result.package_id);
      })
      .catch(function(error){
        packageStatus.className = 'toast err';
        packageStatus.textContent = 'Ошибка генерации: ' + error.message;
      });
  }

  function startTelegramPublish(){
    if (!currentPackageId) return;
    publishResult.textContent = 'PUBLISHING_TELEGRAM...';

    fetch('/api/publish-packages/' + currentPackageId + '/publish', {
      method:'POST',
      headers:{'Content-Type':'application/json'},
      body:JSON.stringify({targets:['telegram']})
    })
      .then(parseJsonResponse)
      .then(function(result){
        publishResult.textContent = JSON.stringify(result, null, 2);
        pollPackage(currentPackageId);
      })
      .catch(function(error){ publishResult.textContent = 'ERROR: ' + error.message; });
  }
  ...
  publishTelegram.addEventListener('click', startTelegramPublish);
  movieTitle.addEventListener('input', refreshPreview);
  movieYear.addEventListener('input', refreshPreview);
  refreshPreview();
})();
</script>
</body>
