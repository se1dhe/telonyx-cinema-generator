...
APP_VERSION = "dialogue-shorts-v1-2026-05-18"
...
def write_title_ass(path: Path, movie_title: str, movie_year: str, duration: float) -> None:
    title = ass_escape(movie_title.strip() or "MOVIE")
    year = ass_escape(movie_year.strip() or "YEAR")
    start_text = f"{{\\move(-650,1518,78,1518,0,620)\\fad(160,260)}}{title}\\N{{\\fs38\\alpha&H18&}}{year}"
    outro_start = max(0.0, duration - 3.1)
    outro_text = f"{{\\move(78,1518,-720,1518,300,1050)\\fad(120,360)}}{title}\\N{{\\fs38\\alpha&H18&}}{year}"
...
Style: Title,DejaVu Sans,54,&H00FFFFFF,&H000000FF,&H99000000,&HAA000000,-1,0,0,0,100,100,0,0,1,3,1,1,70,70,300,1

[Events]
Format: Layer, Start, End, Style, Name, MarginL, MarginR, MarginV, Effect, Text
Dialogue: 2,{ass_time(0.18)},{ass_time(min(duration, 4.4))},Title,,0,0,0,,{start_text}
Dialogue: 2,{ass_time(outro_start)},{ass_time(duration)},Title,,0,0,0,,{outro_text}
"""
...
def render_video(job_id: str) -> None:
    ...
    try:
        ...
        write_state(job_id, {"progress": 22, "duration": duration, "message": "Готовлю выезжающий титр"})
        ...
        vf = "scale=1080:1920:force_original_aspect_ratio=increase,crop=1080:1920,eq=contrast=1.07:saturation=1.08:brightness=-0.018,unsharp=5:5:0.55:3:3:0.25,subtitles=" + str(title_ass)
        if segments:
            vf += ",subtitles=" + str(subs_ass)
