...
APP_VERSION = "dialogue-shorts-v3-premium-title-watermark-2026-05-18"
...
def write_title_ass(path: Path, movie_title: str, movie_year: str, duration: float) -> None:
    # Премиальный титр:
    # - расположен ниже и левее, в безопасной зоне vertical 1080x1920;
    # - выкатывается в начале, висит весь ролик и закатывается только в конце;
    # - справа постоянно держится фирменный TELONYX trademark.
    title = ass_escape((movie_title.strip() or "MOVIE").upper())
    year = ass_escape(movie_year.strip() or "YEAR")

    enter_end_ms = 760
    outro_start = max(0.8, duration - 1.25)
    outro_move_end_ms = 940
    ...
    panel_y = 1534
    panel_hidden_x = -850
    panel_shape = r"{\p1}m 0 0 l 574 0 l 574 132 l 0 132{\p0}"

    accent_x = 54
    accent_y = 1534
    accent_hidden_x = -120
    accent_shape = r"{\p1}m 0 0 l 5 0 l 5 132 l 0 132{\p0}"

    # Год больше не торчит как отдельный дешёвый блок: он работает как маленький production-tag.
    title_text = f"{{\\fsp1.8\\blur0.35}}{title}\\N{{\\fs25\\fsp5\\alpha&H24&}}{year}  /  CINEMA MOMENT"
...
Style: Title,DejaVu Sans,38,&H00F6F2EA,&H000000FF,&HAA020203,&H00000000,-1,0,0,0,100,100,0,0,1,1.4,0.9,1,0,0,0,1
Style: Panel,DejaVu Sans,36,&H94111114,&H000000FF,&H00000000,&H00000000,0,0,0,0,100,100,0,0,1,0,0,7,0,0,0,1
Style: Accent,DejaVu Sans,36,&H001ED8D2,&H000000FF,&H001ED8D2,&H00000000,0,0,0,0,100,100,0,0,1,0,0,7,0,0,0,1
Style: Mark,DejaVu Sans,22,&H78FFFFFF,&H000000FF,&H66000000,&H00000000,-1,0,0,0,100,100,0,0,1,0.8,0.4,5,0,0,0,1

[Events]
Format: Layer, Start, End, Style, Name, MarginL, MarginR, MarginV, Effect, Text
Dialogue: 0,{ass_time(0.00)},{ass_time(outro_start)},Panel,,0,0,0,,{{\\move({panel_hidden_x},{panel_y},{panel_x},{panel_y},0,{enter_end_ms})\\fad(120,0)}}{panel_shape}
Dialogue: 1,{ass_time(0.00)},{ass_time(outro_start)},Accent,,0,0,0,,{{\\move({accent_hidden_x},{accent_y},{accent_x},{accent_y},0,{enter_end_ms})\\fad(120,0)}}{accent_shape}
Dialogue: 2,{ass_time(0.00)},{ass_time(outro_start)},Title,,0,0,0,,{{\\move({title_hidden_x},{title_y},{title_x},{title_y},0,{enter_end_ms})\\fad(120,0)}}{title_text}
Dialogue: 0,{ass_time(outro_start)},{ass_time(duration)},Panel,,0,0,0,,{{\\move({panel_x},{panel_y},{panel_hidden_x},{panel_y},0,{outro_move_end_ms})\\fad(0,180)}}{panel_shape}
Dialogue: 1,{ass_time(outro_start)},{ass_time(duration)},Accent,,0,0,0,,{{\\move({accent_x},{accent_y},{accent_hidden_x},{accent_y},0,{outro_move_end_ms})\\fad(0,180)}}{accent_shape}
Dialogue: 2,{ass_time(outro_start)},{ass_time(duration)},Title,,0,0,0,,{{\\move({title_x},{title_y},{title_hidden_x},{title_y},0,{outro_move_end_ms})\\fad(0,190)}}{title_text}
...
def render_video(job_id: str) -> None:
    ...
    try:
        ...
        write_state(job_id, {"progress": 22, "duration": duration, "message": "Готовлю премиальный титр и TELONYX trademark"})
