...
TMDB_API_KEY = os.getenv("TMDB_API_KEY", "").strip() or os.getenv("TMDB_BEARER_TOKEN", "").strip()
...
def write_title_ass(path: Path, movie_title: str, movie_year: str, movie_genre: str, duration: float) -> None:
    title = ass_escape((movie_title.strip() or "MOVIE").upper())
    year = ass_escape(movie_year.strip() or "YEAR")
    genre = ass_escape(movie_genre.strip() or "")
...
def video_filter(title_ass: Path, subs_ass: Path | None) -> str:
    chain = ["eq=contrast=1.07:saturation=1.08:brightness=-0.018", "unsharp=5:5:0.55:3:3:0.25", "subtitles='" + ass_filter_path(title_ass) + "'"]
    if subs_ass:
...
def render_video(job_id: str) -> None:
    ...
    try:
        ...
        write_title_ass(title_ass, state["movie_title"], state["movie_year"], state.get("movie_genre", ""), duration)
...
def render_tiktok_video(job_id: str) -> None:
    ...
    try:
        ...
        filter_complex = (
            f"[1:v]scale=100:-1,format=rgba,geq=r='r(X,Y)':g='g(X,Y)':b='b(X,Y)':a='if(lte(pow(X-W/2,2)+pow(Y-H/2,2),pow(W/2,2)),255,0)',colorchannelmixer=aa={logo_opacity:.2f}[logo];"
            f"[0:v]scale=1080:1920:force_original_aspect_ratio=increase,"
            f"crop=1080:1920,eq=contrast=1.07:saturation=1.08:brightness=-0.018,"
            f"unsharp=5:5:0.55:3:3:0.25[bg];"
            f"[bg][logo]overlay=W-w-30:(H-h)/2+8*sin(2*PI*t/3):eval=frame"
        )
        run_cmd(job_id, [
            FFMPEG, "-y",
            "-i", str(input_path),
...
def search_movie_metadata(movie_title: str, movie_year: str = "") -> dict[str, Any]:
    """Search movie metadata (year and genre) via TMDB API."""
    if not TMDB_API_KEY:
    ...
    # Support both v4 Bearer token and v3 api_key
    use_bearer = len(TMDB_API_KEY) > 40 or TMDB_API_KEY.startswith("ey")
    if use_bearer:
    ...
    try:
        response = requests.get("https://api.themoviedb.org/3/search/movie", params=params, headers=headers, timeout=10)
        ...
        # If multiple results, return them all for user to choose
        if len(results) > 1:
            alternatives = []
            for alt in results[1:5]:  # Max 4 alternatives
                alt_date = alt.get("release_date", "")[:4] if alt.get("release_date") else ""
                alternatives.append({
                    "title": alt.get("title", ""),
                    "year": alt_date,
                    "original_title": alt.get("original_title", "")
                })
            return {
                "year": year,
...
function tiktokPollJob(id){fetch('/api/tiktok-jobs/'+id,{cache:'no-store'}).then(r=>r.json()).then(job=>{tiktokSetProgress(job.progress||0);tiktokStatus.textContent=job.message||job.status;tiktokLog.textContent=JSON.stringify(job,null,2);if(job.status==='done'){currentTikTokJobId=id;tiktokSetToast('Готово! Видео с логотипом l2watcher','ok');tiktokDownload.href=job.download_url;tiktokDownload.style.display='block';if(job.suggested_tags&&job.suggested_tags.length){tiktokTags.style.display='block';tiktokTags.innerHTML='<div class=\"desc\"><b>'+esc(job.youtube_meta_title||'Lineage 2')+'</b></div><div class=\"taglist\">'+job.suggested_tags.map(t=>'<span>#'+esc(t)+'</span>').join('')+'</div>'}tiktokRenderBtn.disabled=false;return}if(job.status==='failed'){tiktokSetToast('Ошибка: '+(job.message||'unknown'),'err');tiktokRenderBtn.disabled=false;return}setTimeout(()=>tiktokPollJob(id),1500)}).catch(e=>{tiktokSetToast('Ошибка статуса: '+e.message,'err');tiktokRenderBtn.disabled=false})}
function startTikTokRender(){const url=tiktokUrlInput.value.trim();const file=tiktokVideoInput.files.length;if(!url&&!file)return tiktokSetToast('Укажи YouTube ссылку или выбери видеофайл.','err');tiktokDownload.style.display='none';tiktokTags.style.display='none';tiktokRenderBtn.disabled=true;tiktokSetToast(url?'Скачиваю с YouTube...':'Загружаю файл...','warn');tiktokSetProgress(3);const fd=new FormData(tiktokForm);fetch('/api/tiktok-jobs',{method:'POST',body:fd}).then(r=>r.json()).then(r=>{tiktokSetToast('Задача создана: '+r.job_id,'ok');tiktokPollJob(r.job_id)}).catch(e=>{tiktokSetToast('Ошибка старта: '+e.message,'err');tiktokRenderBtn.disabled=false})}
...
def movie_search(title: str, year: str = "") -> dict[str, Any]:
    """Search movie metadata (year and genre) via TMDB API."""
    if not title or len(title.strip()) < 2:
        raise HTTPException(status_code=400, detail="Title too short")
    return search_movie_metadata(title.strip(), year.strip())


@app.post("/api/jobs")
def create_job(background_tasks: BackgroundTasks, video: UploadFile = File(...), movie_title: str = Form(...), movie_year: str = Form(...), movie_genre: str = Form(""), language: str = Form("auto"), subtitles_enabled: bool = Form(False)) -> dict[str, Any]:
